...
class SrcCollect {
	...
	# My custom get render code from - to line number of file
	public function getSourceToFrom($start, $end, $file) {
		$this->setStart($start);
		$this->collect('php', $this->getSourceToLine($end, $file));  
		//$this->getSourceToLine($end, $file);  
	...
	public function getSnippetFromFile($str_start, $str_end, $file, $indent=1) {
		...
		$start =strpos($code, $str_start);  //Find the position of the first occurrence of a substring in a string
