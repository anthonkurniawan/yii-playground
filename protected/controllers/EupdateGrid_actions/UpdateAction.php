...
class UpdateAction extends CAction
{
  ...
  public function run()
  {
    ...
    // Get the Model class
    #$modelClass = ucfirst( $controller->getId() );   //gw ganti ke "modelName"
	$modelClass = $this->modelName;
														
    // Create the model
    $model = $controller->loadModel();  
	#-------------------------------------------------------------------------
    // $findModel2 = TblData::model()->findByPk($_GET['id']);	
		
		// if($findModel2 != null) {
			// $model2 = $findModel2;
			// $findModel2->propic_id !=null ? $model3 = TblPic::model()->findByPk($findModel2->propic_id) : $model3= new TblPic;
		// }
		// else {
			// $model2= new TblData;
			// $model3= new TblPic;
      ...
      if( $model->validate() )
      {
        ...
        if( $model->save() )
        {
          ...
          if( $this->callback !== null &&
              method_exists( $controller, $this->callback ) )  //echo $this->callback;
            $controller->{$this->callback}( "tona");
    ...
    {																	
      ...
      echo CJSON::encode( array(
        ...
        'content' => $controller->renderPartial( $this->ajaxView, array(
         'model' => $model,
		 //'model'=>$model, 'model2'=>$model2, 'model3'=>$model3,          //XXXXXXXXXXXXXXXXXXXXXX
