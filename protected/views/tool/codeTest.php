...
$(document).ready(function(){ 
	//$("#form").html( panel_btn("form"));
	//$('textarea').parent().html(panel_btn("form"));		NANTI BUAT WIDGET AJA BIAR SIMPLE
});
</script>

<div id="form" class="view2" style="min-width:700px; max-width:1200px">

<?php $this->widget('myPanelbar', array('selector'=>'cmd', 'title'=>'', 'content'=>'', ));  ?>

<?php echo CHtml::beginForm(); ?>
<?php echo CHtml::textarea("sourcecode", $content, array("id"=>"cmd", 'placeholder'=>"type code..", "rows"=>"10", "style"=>"width:1000px;", "class"=>"cmd_line" )); ?>
<input type="hidden" name="to" value="interpretcode" />
<input type="hidden" name="nonce" value="<?php echo $this->get_nonce(); /* Russell, 2012-11-10 */ ?>" />
<input type="submit" value="Interpret the code" class="btn" />

<?php
echo CHtml::ajaxSubmitButton(
	'Submit Code(ajax)',        
	array(),  // or with params - array('/widget_CgridView/view', 'id'=>'1', ), 
	array(				
		...
		'error'=> 'function(xhr, status, errorThrown) {  
			$("#result").html("<b class=txtAtt>Status : "+ xhr.status +"-"+ errorThrown + "</b><br><br>" + xhr.responseText);		// xhr.statusText sama dgn errorThrown
		}',
		'beforeSend' => 'function() {  
			//$("#result").html(""); $("#result").addClass("loading box-load"); 
			$("#load").show();
		}',
		'complete' => 'function() { 
			$("#result").parent().addClass("Mybox");
			//$("#result").removeClass("loading box-load"); 
			$("#load").hide();
		}', 
	),
	#Note, if you specify the 'success' option, the "update/replace" options will be ignored.
	#array('success'=>'function(html){ $("#req1").html(html) }' ),		// $("#req1").addClass("bar-loading");
	array('class'=>'btn')	
);
?>	
...
<div class="left">Result :</div> <div id="load" class="left loading box-load" style="display:none">&nbsp;&nbsp;</div>
<div class="clear"></div>

<div style="min-width:800px; max-width:1000px">

<pre id="result" style="background-color:purple; border:1px solid black; color:white"></pre>  <!--- VIA AJAX --->

   <!---  Submit non ajax req ---->
<?php 
if (isset($_POST['sourcecode'])) 
	include(Yii::app()->viewPath.'/tool/_codesource.php');
 ?>
</div>

<a href="#" onClick=" alert( $(this).attr('title').replace('param', 'x') );" title="param #1 xx">test</a>
<hr>
<pre>
 * @class RegExp
 *
